...
- 定义
  ...
  集成算法大致可以分为：Bagging，Boosting 和 Stacking 三大类型。
...
- **标称型数据**（Nominal Data）
  ...
  它用于分类或标记**不同的类别或组别**,数据点之间并**没有数值意义上的距离或顺序**。例如，颜色（红、蓝、绿）、性别（男、女）或产品类别（A、B、C）。
...
- **连续型数据**（Continuous Data）
  ...
  表示在某个范围内可以取任意数值的测量，这些数据点之间有明确的数值关系和距离。例如，温度、高度、重量等。
...
- 定义
  ...
  回归，一般都是指线性回归（linear regression）。线性回归意味着可以将输入项分别乘以一些常量，再将结果加起来得到输出。线性回归是机器学习中一种**有监督学习**的算法,**回归问题**主要关注的是**因变量**(需要预测的值)和一个或多个数值型的**自变量**(预测变量)之间的关系。

  需要预测的值:即目标变量,**target,y**

  影响目标变量的因素:$X_1,X_2...X_n$,可以是连续值也可以是离散值

  因变量和自变量之间的关系:即**模型(model)**,就是我们要求解的
...
- 说明
  ...
  **人工智能中的线性回归**:数据集中,往往找不到一个完美的方程式来100%满足所有的y目标,我们就需要**找出一个最接近真理的方程式**。
...
$$
根据求导原理，求得w=0.795时损失函数取得最小值，$y=0.795x+0$
...
karen的各项指标是:被爱:11 学习指数:14  抗压指数:8  运动指数:10  饮食水平:5  金钱:10 心态:8 压力:1
...
**如果该方程有解，则解出 权重 $w(w_1,w_2...w_8)$ 然后带入即可求出karen的健康程度。**
...
- 方法一（比较繁琐）
  ...
  $loss=[(h(x_1)-y_1)^2+(h(x_2)-y_2)^2+...(h(x_n)-y_n)^2]/n\\=\frac{1}{n} \textstyle\sum_{i=1}^{n}(h(x_{i})-y_{i})^{2}\\=\frac{1}{n}(XW-y)^2\\=\frac{1}{2}(XW-y)^2  这就是传说中的最小二乘法公式$
...
- 方法二（链式求导法则，深度学习常用）
  ...
  **内部函数的导数：**$ \frac{\partial f}{\partial W} = X^T $
  ...
  karen的各项指标是:被爱:11 学习指数:14  抗压指数:8  运动指数:10  饮食水平:5  金钱:10 权利:8 压力:1
...
- 自我理解
  ...
  梯度下降是一种在机器学习和深度学习中常用的优化算法。可以把它想象成一个找山底的过程。

  假设一个人被放在一座山上，他的目标是找到山的最低点。但是，无法看到全局地形，只能感知到自己脚下位置的坡度。梯度下降就是利用这种局部信息来决定应该往哪个方向走以及走多远。

  在这个过程中，“梯度”指的是函数（比如山的高度）在某一点的变化率或斜率，它指示了函数值增加最快（坡度最陡峭）的方向。“下降”则意味着我们朝着与梯度相反的方向移动，因为我们的目标是最小化某个函数（比如找到山谷的最低点）。

  具体到数学上，梯度下降通过迭代更新参数（如模型中的权重）来逐渐减少损失函数（衡量预测结果与实际结果之间差距的函数）的值。每次迭代时，都会计算当前参数设置下的梯度，并沿着负梯度方向调整参数值，直到达到一个局部最小值或者满足某种停止条件。

  简单来说，梯度下降就像是一个人在下山，不断根据脚下的斜坡调整步伐大小和方向，最终找到山谷底部的过程。

- 实际定义

  机器学习中，**梯度表示损失函数对于模型参数的偏导数**。具体来说，对于每个可训练参数，**梯度告诉我们在当前参数值下，沿着每个参数方向变化时，损失函数的变化率**。通过计算损失函数对参数的梯度，梯度下降算法能够根据梯度的信息来调整参数，**朝着减少损失的方向更新模型**，从而逐步优化模型，使得模型性能更好。

**梯度下降法(**Gradient Descent)，相比于最小二乘法(MSE)计算量更小，速度更快。

#### 7.2 梯度下降步骤

![gradient_1](https://github.com/ljgit1316/Picture_resource/blob/main/Machine_Pic/gradient_1.png)

1、Random随机数生成初始W，一**般随机生成一组成正太分布的数值$w_0,w_1,w_2....w_n$**，或者自己拟定初始W值。
...
w=w-0.01*w为0.2时的梯度(导数) 算出来是 0.198

w=w-0.01*w为0.198时的梯度(导数) 算出来是 0.19602

。。。。。。。。

就这样一直更新下去,会在真实值附近,我们可以控制更新的次数

**若w在目标值左边，导数是负数 ，减去负数就是加，往右移动**

**若w在目标值右边，导数是正数 ，减去正数就是减，往左移动**
...
- 作用
  ...
  一般情况下学习率在整体迭代过程中是不变，但是也可以设置成**随着迭代次数增多学习率逐渐变小**

  例如：（learning_rate/(固定数值+迭代次数)）
...
- 代码
  ...
  for i in range(step):
      a = t0 / (t1 + i)  # 自适应学习率
      gradient = 2 * (w1 - 3.5) - 4.5  # 损失函数导数（梯度）
      w1 = w1 - a * gradient  # 更新w1
      loss = (w1 - 3.5) ** 2 - 4.5 * w1 + 10  # 计算损失值
      print(f'第{i + 1}次迭代，w1={w1},更新后的损失值={loss}')
  ```
...
- 公式
  ...
  **求梯度时，同时更新w1,w2,在数学角度上，就是求w1、w2的偏导数**
...
- 代码
  ...
  for i in range(step):
      a=t0/(t1+i)#自适应学习率
      s_w1=w1#保存起来  防止梯度下降过程中w1和w2的值发生改变
      s_w2=w2
  
      g_w1=2*(s_w1-3.5)+3*s_w2-4.5#w1方向的梯度（导数）
      g_w2=2*(s_w2-2)+3*s_w1+2#w2方向的梯度（导数）
  
      w1=w1-a*g_w1#更新w1
      w2=w2-a*g_w2#更新w2
      loss=(w1-3.5)**2+(w2-2)**2+3*w1*w2-4.5*w1+2*w2+10#计算损失值
      print(f'第{i+1}次迭代，w1={w1},w2={w2},更新后的损失值={loss}')
  ```

  ![two_yuan_end](https://github.com/ljgit1316/Picture_resource/blob/main/Machine_Pic/two_yuan_end.png)

#### 7.6 sklearn API实现梯度下降
...
- 三种梯度下降区别
  ...
  - **Batch Gradient Descent (BGD)**: 每一次迭代都会使用**全部的训练样本计算梯度来更新权重**。这意味着每一步梯度更新都是基于整个数据集的平均梯度。这种方法的优点是每次更新的方向是最准确的，但**缺点是计算量大且速度慢**，尤其是在大数据集上。
  - **Mini-Batch Gradient Descent (MBGD)**: 这种方法介于批量梯度下降和随机梯度下降之间。它不是用全部样本也不是只用一个样本，而是**每次迭代从数据集中随机抽取一小部分样本（例如，从500个样本中选取32个）**，然后基于这一小批样本的平均梯度来更新权重。这种方法在准确性和计算效率之间取得了一个平衡。
...
- 原理
  ...
  每一次迭代都会使用**全部的训练样本计算梯度来更新权重**，并据此更新模型参数。此方法不常用，没有官方API，可自己实现。

- 如何更新

  设我们有一个包含 \( m \) 个训练样本的数据集 $\{(x^{(i)}, y^{(i)})\}_{i=1}^{m}$，其中 $ x^{(i)} $是输入特征，$ y^{(i)} $ 是对应的标签。我们的目标是最小化损失函数 $J(\theta) $ 相对于模型参数 $ \theta $ 的值。
...
- 适用场景
  ...
  - **不需要实时更新**：如果模型不需要实时更新，例如在**离线训练场景**下，批量梯度下降是一个合理的选择。
...
- 数学公式
  ...
  设我们有一个包含 \(m\) 个样本的数据集 $\{(x^{(i)}, y^{(i)})\}_{i=1}^m$，其中 $x^{(i)}$ 是第 $i$ 个样本的特征向量，$y^{(i)}$ 是对应的标签。对于线性回归问题，损失函数 $J(\theta)$ 可以定义为均方误差 (Mean Squared Error, MSE)：
  $J(\theta) = \frac{1}{2}(h_\theta(x^{(i)}) - y^{(i)})^2$
  其中 $h_\theta(x^{(i)}) = \theta^T x^{(i)}$是模型对第 $i$个样本的预测值。

  梯度 $\nabla J(\theta)$ 对于每个参数 $\theta_j$ 的偏导数可以表示为：
...
- 代码实现
  ...
  #训练模型的参数
  print('训练模型的回归权重系数：',sgd.coef_)
  print('训练模型的偏置：',sgd.intercept_)
  #模型评估
...
#### 8.3正则化
  ...
  正则化就是防止过拟合，增加模型的**鲁棒性**，鲁棒是Robust 的音译，也就是强壮的意思。就像计算机软件在面临攻击、网络过载等情况下能够不死机不崩溃，这就是软件的鲁棒性,鲁棒性调优就是让模型拥有更好的鲁棒
...
第一个更好，因为下面的公式是上面的十倍，当w越小公式的容错的能力就越好。我们都知道人工智能中回归是有误差的,为了把误差降低而拟合出来的一个接近真实的公式,比如把一个测试数据[10,20]带入计算得到的值跟真实值会存在一定的误差,但是第二个方程会把误差放大,公式中 
 $y = W^Tx$ ，当 $x$ 有一点错误，这个错误会通过w放大。但是w不能太小，当w太小时(比如都趋近0)，模型就没有意义了，无法应用。想要有一定的容错率又要保证正确率就要由**正则项**来发挥作用了!
...
其实 $L1$ 和 $L2$ 正则的公式在数学里面的意义就是范数,代表空间中向量到原点的距离

当我们把多元线性回归损失函数加上 $L2$ 正则的时候，就诞生了Ridge岭回归。当我们把多元线性回归损失函数加上 $L1$ 正则的时候，就孕育出来了Lasso回归。其实 $L1$ 和 $L2$ 正则项惩罚项可以加到任何算法的损失函数上面去提高计算出来模型的泛化能力的。
